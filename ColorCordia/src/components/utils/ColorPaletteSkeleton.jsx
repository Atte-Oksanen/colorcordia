  ...
  return (
    <div className="flex items-center justify-center h-screen">
      <div className="md:w-[95%] mt-5 h-fit">
        ...
        <div>
          <div className="md:hidden grid grid-cols-1 grid-rows-5 rounded-md overflow-hidden">
            {props.colors.map(color => {
              return (
                <div className="grid grid-rows-[2fr_1fr] grid-cols-1" key={Math.random()}>
                  ...
                  <div className="px-5 py-2 text-center" key={Math.random()}>{color.hex} - {color.name}</div>
                </div>
              )
            })}
          </div>
