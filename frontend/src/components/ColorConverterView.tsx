...
import { HSV, NCS } from "../types/colorTypes"

const ColorConverterView = ({ setMessage }: { setMessage: React.Dispatch<React.SetStateAction<Message | null>> }) => {
  const [startUnit, setStartUnit] = useState('Hex')
  const [startValue, setStartValue] = useState<string>('')
  const [endUnit, setEndUnit] = useState('HSV')
  const [endValue, setEndValue] = useState('')
  const [showComparison, setShowComparison] = useState(false)
  const [comparableColors, setComparable] = useState<({ color: string, name: string } | NCS)[]>([])
  ...
  const getPlaceHolderFormat = (unit: string) => {
    switch (unit) {
      case 'Hex':
        return 'e.g. #85d2d6'
      case 'RGB':
    ...
    switch (startUnit) {
      case 'Hex':
        ...
        switch (endUnit) {
          ...
          case 'NCS': {
            const ncs: NCS = await hexToNcs(startValue.substring(1))
            setEndValue(ncs.ncs)
  ...
  const renderComparison = () => {
    ...
    const hexColor = comparableColors[0] as { color: string; name: string; }
    if (showComparison) {
      const ncsColor = comparableColors[1] as NCS
      return (
        <div className="md:w-1/2 md:px-0 px-2 mx-auto h-full">
          <div className="grid grid-cols-2 h-1/2 rounded-lg overflow-hidden mt-8">
            <div style={{ background: `${hexColor.color}` }}></div>
            <div style={{ background: `${ncsColor.hex}` }} ></div>
          </div>
          <div className="grid grid-cols-2 text-center">
            <div>{hexColor.name}</div>
            <div>{ncsColor.ncs}</div>
          </div>
          <div>
    ...
    return (
      <div className="md:w-1/2 md:px-0 px-2 mx-auto h-full">
        <div className="h-1/2 rounded-lg overflow-hidden mt-8" style={{ background: `${hexColor.color}` }}></div>
        <div className="text-center">{hexColor.name}</div>
      </div>
    )
  }
  return (
